...
export class ParallelCarsGame extends Game {
  ...
  constructor(
    canvasId: string,
    carThreads: number,
    laneThreads: number = 4
  ) {
    super(canvasId);
    const suggestedThreads = laneThreads;
    const usedThreads = Math.min(carThreads, 10);
    const boxCount = 10;

    for (let i = 0; i < boxCount; i++) {
      const gamePackage = LoadPackage(1000 + i, 1100, 100 + i * 60);
      this.addGameObject(gamePackage);
      this._packages.push(gamePackage);
    }

    for (let i = 0; i < suggestedThreads; i++) {
      this.addGameObject(new Lane(i, 100, 110 + 120 * i, 1000, 90));
    }
  ...
  private pickUpPackage() {
    const pkg = this._packages.filter(
      item => !item.pickedUp && !item.delivered
    )[0];
    if (pkg) {
      pkg.PickUp();
    }
